...
// class L1WordWriteReq(implicit p: Parameters) extends BoomBundle()(p) {
...
//     val resp_block = Output(Vec(memWidth, Vec(nWays, Vec(refillCycles, Bits((encRowBits).W)))))
//   })
...
// {
//   val waddr = io.write.bits.addr >> (rowOffBits + log2Ceil(refillCycles))
//   val write_wordOffset = io.write.bits.addr(rowOffBits + log2Ceil(refillCycles) - 1, rowOffBits)
//   for (j <- 0 until memWidth) {

//     val raddr = io.read(j).bits.addr >> (rowOffBits + log2Ceil(refillCycles))
//     val read_wordOffset = io.read(j).bits.addr(rowOffBits + log2Ceil(refillCycles) - 1, rowOffBits)
//     for (w <- 0 until nWays) {
//       for (v <- 0 until refillCycles) {
//         val (array, omSRAM) = DescribedSRAM(
//                 name = s"array_${w}_${v}_${j}",
//                 desc = "Non-blocking DCache Data Array",
//                 size = nSets,
//                 data = Vec(rowWords, Bits(encDataBits.W))
//         )
//         when (io.write.bits.way_en(w) && io.write.valid && (write_wordOffset === v.U)) {
...
//         }
//         io.resp_block(j)(w)(v) := RegNext(array.read(raddr, io.read(j).bits.way_en(w) && io.read(j).valid).asUInt)
//       }
//       io.resp(j)(w) := io.resp_block(j)(w)(read_wordOffset)
//     }
//     io.nacks(j) := false.B
//   }
// }
...
//   val victimCacheParams = VictimCacheParams(rowBits = cfg.rowBits, nSets=1, nWays=16, paddrBits=32, blockBytes=cfg.blockBytes)
...
//   val s2_victim_data = Wire(Vec(refillCycles, Vec(memWidth, Vec(nWays, UInt(encRowBits.W)))))
//   for (i <- 0 until memWidth) {
//     for (w <- 0 until nWays) {
...
//   // ******************** added signals **********************
//   val s2_victim_data = Wire(Vec(memWidth, Vec(nWays, Vec(refillCycles, UInt(encRowBits.W)))))
//   for (i <- 0 until memWidth) {
//     for (v <- 0 until refillCycles) {
//       for (w <- 0 until nWays) {
//         s2_victim_data(i)(w)(v) := data.io.resp_block(i)(w)(v)
//       }
//     }
//   }
//   victimCache.io.evicted_data.valid <> mshrs.io.req_2victim.valid
//   // ************************* end ***************************
...
//   victimCache.io.req <> mshrs.io.req_2victim
