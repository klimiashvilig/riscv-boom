...
//   val victimCacheParams = VictimCacheParams(rowBits = icacheParams.rowBits, nSets=1, nWays=16, paddrBits=32, fetchBytes=icacheParams.fetchBytes)
...
//   when (victim_refill_done) { 
//     invalidated := true.B               // This just tells us that we don't need to update L1 using L2 repsonse
//     victim_refill_valid := false.B }


//   val victim_refill_paddr = RegEnable(io.s1_paddr, s1_valid && !(refill_valid || s2_miss))    // physical address for victim $
//   val victim_refill_idx = victim_refill_paddr(untagBits-1,blockOffBits)
...
//       val wen = (refill_one_beat && !invalidated) && repl_way === i.U && refill_cnt === j.U
...
//   val s2_victim_data = s2_victim_dout(repl_way).asUInt
//   val s2_vb = vb_array(Cat(repl_way, refill_idx))
//   victimCache.io.evicted_data.valid := s2_vb && s2_miss && !refill_valid && !io.s2_kill
...
//   // ******** Added signals *********
//   victimCache.io.req.valid := s2_miss && !refill_valid && !io.s2_kill // request victim cache access if we missed and haven't already requested it yet
//   victimCache.io.req.bits.addr := (victim_refill_paddr >> blockOffBits) << blockOffBits  // the physical address requested
//   // ************* end **************
